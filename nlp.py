...
def review_clean(df):
    ...
    # words_split
    df['精选评论'] = df['精选评论'].astype(str)
    df['精选评论'] = df['精选评论'].apply(lambda x: ' '.join(jieba.cut(x)))
...
def nlp_score(df):
    ...
    df_nlp['target'] = df_nlp['评分'].map(lambda x: score_trans(x))
    df_model = df_nlp.dropna()

    x_train, x_test, y_train, y_test = train_test_split(df_model['精选评论'], df_model['target'],
    ...
    x_train2 = pd.concat([x_train, x_tmp, x_tmp, x_tmp])
    y_train2 = pd.concat([y_train, y_tmp, y_tmp, y_tmp])
    ...
    df_nlp['nlp_score'] = df_nlp['精选评论'].map(lambda x: pred_score(classifier, tv, x))
